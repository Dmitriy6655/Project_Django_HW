...
def client_goods(request):
    ...
    if request.method == "POST":
        ...
        COUNT_DAYS30 = 30
        start2 = datetime.date.today() - datetime.timedelta(days=COUNT_DAYS7)
        orders30 = Order.objects.filter(client_id=client_id, create_at__gte=start2)
        ...
        context = {
            'title': title,
            'count_days7': COUNT_DAYS7,
            'count_days30': COUNT_DAYS30,
            'count_days365': COUNT_DAYS365,
            'client': client,
            'orders7': orders7,
            'orders30': orders30,
            'orders365': orders365,
