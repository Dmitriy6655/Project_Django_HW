...
from hw2.models import Client, Order, Goods
...
def test(request):
    ...
    logger.info(f'context: {context}')
    return render(request, 'online_store/test.html', context)
...
def get_catalog(request):
    ...
    logger.info(f'context: {context}')
    return render(request, 'online_store/catalog.html', context=context)
...
def good_full(request: HttpRequest, good_pk: int) -> HttpResponse:
    ...
    return render(request, "online_store/good_full.html", context=context)
